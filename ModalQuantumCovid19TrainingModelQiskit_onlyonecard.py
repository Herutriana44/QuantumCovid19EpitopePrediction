...
@app.function(
    gpu="B200",  # Single B200 GPU for optimal performance
    timeout=3600*4,  # 4 hour timeout for maximum training
    volumes={"/results": volume},
    cpu=16,  # CPU cores for single GPU support
    memory=32768,  # 32GB memory for single GPU training
...
def train_vqc_gpu(
    ...
    use_gpu: bool = True,
    gpu_type: str = "B200"  # Default to single B200 for optimal performance
) -> Dict[str, Any]:
    """
    ...
    def setup_single_gpu():
        """Setup single GPU training dengan PyTorch"""
        if torch.cuda.is_available():
            num_gpus = torch.cuda.device_count()
            print(f"🚀 Single GPU Setup: {num_gpus} GPU(s) detected")
            
            for i in range(num_gpus):
                print(f"  GPU {i}: {torch.cuda.get_device_name(i)}")
                print(f"  Memory: {torch.cuda.get_device_properties(i).total_memory / 1e9:.1f} GB")
            
            print("ℹ️ Single GPU training mode")
            return False, 1  # No DDP for single GPU
        else:
            print("⚠️ No GPU available, using CPU")
            return False, 0
    
    # Check GPU availability and setup single GPU
    print(f"🚀 Starting VQC training for COVID-19 Epitope Prediction on Modal")
    use_ddp, num_gpus = setup_single_gpu()
            ...
            try:
                ...
                if use_gpu and torch.cuda.is_available():
                    try:
                        ...
                        if gpu_type == "B200":
                            # NVIDIA Blackwell B200 - Ultimate performance dengan single GPU optimization
                            sampler.set_options(
                                device='GPU',
                            ...
                            print("✓ Aer GPU B200 (Blackwell Single GPU) configuration applied")
    ...
    # Setup DataLoaders dengan single GPU optimization
    # Batch size untuk single GPU
    batch_size = 128  # Optimized batch size for single B200 GPU
    print(f"Batch size: {batch_size} for single GPU")
    
    # Single GPU DataLoader setup
    train_loader = DataLoader(
        train_dataset, 
        batch_size=batch_size, 
        shuffle=True,
        num_workers=8,  # Optimal workers for single B200 GPU
        pin_memory=True,
    ...
    print(f"✅ Single GPU DataLoader setup with batch size: {batch_size}")
    ...
    class COVID19VQC(nn.Module):
        def __init__(self, input_dim: int, num_qubits: int, sampler):
            ...
            # Quantum layer - optimized untuk single GPU
            ...
            # Optimize quantum execution untuk single GPU
            qnn = SamplerQNN(
                sampler=sampler,
                circuit=qc,
    ...
    # Initialize model untuk single GPU
    model = COVID19VQC(X.shape[1], num_qubits, sampler)
    
    # Single GPU model setup
    if torch.cuda.is_available():
        model = model.to(device)
        print(f"✅ Single B200 GPU setup")
    
    # Optimizer dengan learning rate untuk single GPU
    base_lr = 0.01
    scaled_lr = base_lr  # No scaling needed for single GPU
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Epochs", unit="epoch"):
        ...
        for batch_idx, (data, target) in enumerate(train_pbar):
            ...
            # No gradient synchronization needed for single GPU
    ...
    results = {
        ...
        # Single GPU training information
        'single_gpu_info': {
            'num_gpus': num_gpus,
            'use_ddp': use_ddp,
            'batch_size': batch_size,
            ...
            'quantum_optimization': 'B200 Single GPU Sampler Configuration'
        },
        'training_history': {
        ...
        'performance_metrics': {
            ...
            'gpu_efficiency': f"{batch_size} samples per batch on single GPU"
        },
    ...
    try:
        # Extract model state (no wrapper needed for single GPU)
    ...
    # No distributed training cleanup needed for single GPU
    ...
    print(f"Single GPU Setup: {num_gpus} GPU")
...
@app.function(
    gpu="B200",  # Single B200 GPU
    timeout=3600*4,  # 4 hours for comprehensive benchmark
    volumes={"/results": volume},
    cpu=16,  # CPU cores untuk single GPU
    memory=32768  # 32GB memory untuk single GPU benchmark
)
def benchmark_samplers() -> Dict[str, Any]:
    ...
    # Test configurations for single GPU
    configs = [
...
def main(
    ...
    gpu_type: str = "B200",  # Single GPU performance
...
if __name__ == "__main__":
    ...
    print("")
    print("🚀 SINGLE GPU TRAINING FEATURES:")
    print("  ✅ Single B200 GPU for optimal performance")
    print("  ✅ Mixed precision training with gradient scaling")
    print("  ✅ Optimized learning rate for single GPU")
    print("  ✅ Memory optimization with automatic cleanup")
    print("  ✅ Enhanced quantum circuit execution")
    print("")
    print("Example commands:")
    print("  # Single GPU training with B200")
    print("  modal run ModalQuantumCovid19TrainingModelQiskit_onlyonecard.py --action=train --gpu-type=B200 --epochs=5")
    print("")
    print("  # Training with single GPU optimization")
    print("  modal run ModalQuantumCovid19TrainingModelQiskit_onlyonecard.py --action=train --gpu-type=B200 --epochs=10 --no-auto-download")
    print("")
    print("  # Comprehensive single GPU benchmark")
    print("  modal run ModalQuantumCovid19TrainingModelQiskit_onlyonecard.py --action=benchmark")
    print("")
    ...
    print("")
    print("Single GPU Optimizations:")
    print("  🔥 Batch size: 128 optimized for single B200 GPU")
    print("  🔥 Learning rate: Optimized for single GPU training")
    print("  🔥 Quantum circuits: Enhanced execution with B200 optimization")
    print("  🔥 Memory: 32GB total with automatic cleanup")
    print("")
    ...
    print("  - modal_sampler_benchmark.json (single GPU benchmark results)")
